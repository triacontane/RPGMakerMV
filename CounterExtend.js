...
(function() {
    ...
    Game_Action.prototype.itemCnt = function(target) {
        ...
        if (this.isMagical()) {
            ...
            target.reserveCounterSkillId(['反撃スキルID', 'CounterSkillId']);
            return target.getCounterCustomRate(['反撃条件', 'CounterCond'], this, target) * cnt;
        }
    };

    Game_Action.prototype.itemMagicCnt = function(target) {
        if (target.isValidMagicCounter() && this.isMagical() && target.canMove()) {
            target.reserveCounterSkillId(['魔法反撃スキルID', 'MagicCounterSkillId']);
            return target.getCounterCustomRate(['魔法反撃条件', 'MagicCounterCond'], this, target) *
                (target.getMagicCounterRate() || target.cnt);
        } else {
            return 0;
        }
    };
