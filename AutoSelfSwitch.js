...
(function() {
    ...
    var param = createPluginParameter('AutoSelfSwitch');
    ...
    Game_Event.prototype.initialize = function() {
        ...
        var dataList = param.list.filter(function(item) {
            return !!this.event().meta[item.noteTag];
        }, this);
        this._autoSelfSwitchIndexList = dataList.map(function(data) {
            return param.list.indexOf(data);
        });
    };
    ...
    Game_Event.prototype.controlSelfSwitch = function(type, value) {
        var key = [$gameMap.mapId(), this.eventId(), type];
        var prevValue = $gameSelfSwitches.value(key);
        if (prevValue !== value) {
